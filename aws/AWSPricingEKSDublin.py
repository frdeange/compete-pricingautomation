...
url = "https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonEKS/current/eu-west-1/index.json"
...
for product in data["products"].values():
    ...
    if "usagetype" in product["attributes"]:
        # Obtener el sku, instanceType, vcpu, memory y storage del producto
        sku = product["sku"]
        instance = product["attributes"]["usagetype"]
        ...
        rows.append([sku, instance, PriceDescription, pricePerUnit])

# Crear un dataframe de pandas con la lista de filas y las columnas deseadas
df = pd.DataFrame(rows, columns=["sku", "usagetype", "PriceDescription", "pricePerUnit"])
...
#filename = "AWS EC2 Dublin Prices " + str(publishDate.year) + str(publishDate.month) + str(publishDate.day) + ".xlsx"
filename = "AWS EKS Dublin Prices.xlsx"
